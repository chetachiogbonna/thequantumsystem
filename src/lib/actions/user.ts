...
export async function getUserByUidWithWalletStatus(uid: string) {
  try {
    ...
    // Check wallet-connect collection for this user
...
export async function updateUserKycStatus(uid: string, status: "approved" | "rejected") {
  ...
  const user = {
    ...
    coinBalance: userDoc.data()?.coinBalance,
  } as User;
...
export async function updateUserCoinBalance(uid: string, newBalance: number) {
  try {
    const userRef = adminDb.collection('users').doc(uid);
